...
# O(n)
def authirinification1(users, user_name, user_password):
    for key, value in users.items():
        if key == user_name:
            if value['password'] == user_password and value['activation']:
                return "Добро пожаловать! Вы прошли активацию."
...
# O(1)
def authirinification2(users, user_name, user_password):
...
# Вторая реализация лучше, т. к. в ней не используется цикл.